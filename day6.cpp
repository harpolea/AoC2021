...
long long evolveLanternfish(vector<int> initialAges, int nDays=80) {
    // we're going to create a map that lists the numbers of lanternfish of each age
    deque<long long> lanternfishAges(9, 0);
    ...
    for (int i = 0; i < nDays; ++i) {
        ...
        auto pregnantFish = lanternfishAges.front();
        lanternfishAges.pop_front();

        // add these back with timers of 6 and add on new fish with timers of 8
        lanternfishAges[6] += pregnantFish;
        lanternfishAges.emplace_back(pregnantFish);
    }

    // sum up queue to get total number of fish
    long long nFish = 0;
    for (const auto& fish : lanternfishAges) {
        nFish += fish;
    }
...
int main() {
    ...
    cout << "After 80 days, there are " << nFish << " lanternfish." << endl;

    nFish = evolveLanternfish(ages, 256);

    cout << "After 256 days, there are " << nFish << " lanternfish." << endl;
}